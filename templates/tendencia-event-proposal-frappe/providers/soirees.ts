...
const DOCTYPE = "Proposal Soiree";

/** Fields stored as Frappe JSON (were Postgres text[] columns). */
const JSON_FIELDS = ["description"];
...
function fromDoc(doc: SoireeDoc): SoireeRow {
  return {
    id: doc.name,
    name: doc.soiree_name,
...
export const provider = {
  ...
  async execute(context: Record<string, any>): Promise<Record<string, any>> {
    const docs = await fetchDocs<SoireeDoc>(DOCTYPE, JSON_FIELDS);
